...
PROPERVAL_TRAIN_SHARDS="${PROPERVAL_TRAIN_SHARDS:-$DATA_DIR/train/shard-000000.tar,$DATA_DIR/train/shard-000001.tar,$DATA_DIR/train/shard-000002.tar,$DATA_DIR/train/shard-000003.tar}"
PROPERVAL_EVAL_SHARDS="${PROPERVAL_EVAL_SHARDS:-$DATA_DIR/train/shard-000004.tar}"
TRAIN_SHARDS="$PROPERVAL_TRAIN_SHARDS"
export TRAIN_SHARDS

require_data

summary_csv="$RESULTS_DIR/wer_summary_properval.csv"
failed_file="$LOGS_DIR/failed_properval_experiments.txt"
touch "$failed_file"
failure_count=0
...
training_args_for() {
  case "$1" in
    ...
    asrinit_layerwise_lr_decay_properval)
      ...
      ;;
    *)
      echo "Unknown proper-validation training experiment: $1" >&2
      return 1
      ;;
  esac
}
...
summary_args_for() {
  case "$1" in
    asr_pretrained_960h_full_properval)
      ...
      ;;
    asrinit_lr1e-6_fp32_properval)
      SUMMARY_ARGS=(--train_setting asr_initialized_properval --learning_rate 1e-6 --freeze_setting none --layerwise_lr_decay false)
      ;;
    asrinit_lr3e-6_fp32_properval)
      SUMMARY_ARGS=(--train_setting asr_initialized_properval --learning_rate 3e-6 --freeze_setting none --layerwise_lr_decay false)
      ;;
    asrinit_lr1e-5_fp32_properval)
      SUMMARY_ARGS=(--train_setting asr_initialized_properval --learning_rate 1e-5 --freeze_setting none --layerwise_lr_decay false)
      ;;
    asrinit_freeze_feature_lr3e-6_fp32_properval)
      SUMMARY_ARGS=(--train_setting asr_initialized_properval --learning_rate 3e-6 --freeze_setting feature_encoder --layerwise_lr_decay false)
      ;;
    asrinit_freeze3_lr3e-6_fp32_properval)
      SUMMARY_ARGS=(--train_setting asr_initialized_properval --learning_rate 3e-6 --freeze_setting first_3_encoder_layers --layerwise_lr_decay false)
      ;;
    asrinit_layerwise_lr_decay_properval)
      SUMMARY_ARGS=(--train_setting asr_initialized_properval --learning_rate 3e-6 --freeze_setting feature_extractor --layerwise_lr_decay 0.9)
      ;;
    *)
      echo "Unknown proper-validation summary experiment: $1" >&2
      return 1
      ;;
  esac
}
...
resolve_experiment_model_path() {
  local experiment="$1"
  if [[ "$experiment" == "asr_pretrained_960h_full_properval" ]]; then
    printf '%s\n' "$MODEL_NAME_OR_PATH"
  else
...
run_experiment() {
  ...
  if [[ "$experiment" == "asr_pretrained_960h_full_properval" ]]; then
    ...
    run_stage "$experiment" training run_properval_training "$experiment" || return $?
  fi
  run_stage "$experiment" inference run_properval_inference "$experiment" || return $?
  run_stage "$experiment" wer_evaluation run_properval_eval "$experiment" || return $?
}
...
echo "Proper-validation train shards: $TRAIN_SHARDS"
echo "Proper-validation eval shards: $PROPERVAL_EVAL_SHARDS"
check_properval_split
...
if [[ -f "$summary_csv" ]]; then
  ...
  echo "Proper-validation WER summary was not created; skipping result summarization."
fi

echo "Proper-validation ASR-init queue finished. Review $failed_file for any recorded failures."
