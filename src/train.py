...
class MyCtcTrainer(Trainer):
    ...
    def _compute_custom_loss(self, model, inputs, labels, outputs, attention_mask):
        ...
        targets = labels[labels.ge(0)].to(torch.long)
        return torch_functional.ctc_loss(
            logits.log_softmax(dim=-1).transpose(0, 1),
            targets,
            logits_lengths,
            target_lengths,
            blank=actual_model.config.pad_token_id,
            reduction="mean",
            zero_infinity=bool(actual_model.config.ctc_zero_infinity),
        )
...
def main():
    ...
    provenance = {
        ...
        "model_name_or_path": args.model_name_or_path,
        "train_shards": data.find_shards(args.train_shards),
        "eval_shards": data.find_shards(eval_shards),
